...
// Déclaration de l'arbre et des tableaux pour les états précédents et suivants
T_Arbre abr = NULL;
GArray *previous_abrs = NULL;
GArray *next_abrs = NULL;

// Déclaration des widgets pour la zone de texte et la zone de dessin
GtkWidget *text_view = NULL;
static GtkWidget *darea = NULL;

// Déclaration des tableaux pour stocker les coordonnées des nœuds graphiques
GArray *noeudsGraphiques = NULL;

// Déclaration des variables pour le zoom et le panoramique
...
// Structure pour stocker les coordonnées d'un nœud graphique
typedef struct {
    double x;
    double y;
    T_Sommet *sommet;
...
// Fonction pour copier un arbre binaire
...
// Fonction pour supprimer l'arbre
...
// Fonction de rappel pour l'événement de dessin
...
// Fonction de rappel pour l'événement de pression du bouton de la souris
...
// Fonction de rappel pour l'événement de relâchement du bouton de la souris
...
// Fonction de rappel pour l'événement de mouvement de la souris
...
// Fonction de rappel pour l'événement de défilement de la souris
static gboolean on_scroll_event(GtkWidget *widget, GdkEventScroll *event, gpointer data) {
    ...
    // Redessiner l'arbre avec le nouveau niveau de zoom et décalage
    gtk_widget_queue_draw(widget);
    if (abr == NULL) {
        return FALSE;
    }
...
// Fonction de rappel pour l'événement de pression de touche
...
// Fonction de rappel pour le bouton "Insérer Élément"
...
// Fonction pour avancer à l'état suivant de l'arbre
...
// Fonction de rappel pour l'événement de pression d'une touche
static gboolean on_key_press_event(GtkWidget *widget, GdkEventKey *event, gpointer user_data) {
    if ((event->state & GDK_CONTROL_MASK) && (event->keyval == GDK_KEY_z)) {
        // Ctrl + Z
        retourner_etat_precedent(widget, user_data);
...
static void rechercher_un_element(GtkWidget *widget, gpointer entry) {
    ...
    if (resultatRecherche != NULL) {
        append_to_message_view(g_strdup_printf("\n"));
        append_to_message_view(g_strdup_printf("Élément trouvé dans l'intervalle [%d; %d].\n", resultatRecherche->borneInf, resultatRecherche->borneSup));
    } else {
        append_to_message_view(g_strdup_printf("\n"));
...
// Fonction pour afficher le père d'un élément donné
...
// Fonction pour afficher le niveau d'un élément donné
...
// Fonction pour réinitialiser l'arbre
static void reinitialiser_arbre(GtkWidget *widget, gpointer data) {
    ...
    recentrer_arbre(darea, abr);
    gtk_widget_queue_draw(darea);
    append_to_message_view(g_strdup_printf("Arbre réinitialisé.\n"));
}
